...
def accuracy(typed, reference):
    ...
    "*** YOUR CODE HERE ***"
    correct = 0
    if len(typed_words) == 0 and len(reference_words) == 0: return 100.0
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    lowdiff = diff_function(typed_word, valid_words[0], limit)
    lowword = valid_words[0]
    for x in valid_words:
        if diff_function(typed_word, x, limit) < lowdiff:
            lowdiff = diff_function(typed_word, x, limit)
            lowword = x
    if lowdiff <= limit:
        return lowword
    else: return typed_word
    # END PROBLEM 5
...
def feline_flips(start, goal, limit):
    ...
    if len(start) == 1 or len(goal) == 1:
        output = abs(len(start) - len(goal))
        if start[0] == goal[0]:
            return 0 + output
        else:
            return 1 + output
    if start[0] == goal[0]:
        output = feline_flips(start[1:], goal[1:], limit)
    else:
        output = 1 + feline_flips(start[1:], goal[1:], limit-1)
    if output > limit:
        return limit + 1
    return output
    # END PROBLEM 6
...
def fastest_words(game):
    ...
    for i in word_indices:
        min = get_times(game)[0][i]
        minp = 0
        for j in player_indices:
            if get_times(game)[j][i] < min:
                min = get_times(game)[j][i]
                minp = j
        out[minp].append(get_words(game)[i])
    return out
    # END PROBLEM 10
